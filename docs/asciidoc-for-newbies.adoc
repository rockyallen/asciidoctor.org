...
- move "Who's using AsciiDoc?" to one of the opening sections
- generating slides
...
While AsciiDoc is designed to be readable in raw form, the intended audience for that format is the writers and editors.
Readers aren't going to appreciate the raw text nearly as much.
Aesthetics matter.
You'll want to apply nice typography with font size that adhere to the ``golden ratio'', colors, icons and images to give it the respect it deserves.
...
Asciidoctor.render_file('sample.ad', :in_place => true)
----
...
[source,ruby]
----
html = Asciidoctor.render_file('sample.ad', :header_footer => true)
puts html
----
